#!/usr/bin/env python3
"""
export/html.txt と export/css.txt を生成するスクリプト。

- index.html を元に、画像(ロゴ・キャラクター)を base64 データURIとして埋め込み、
  js/main.js を末尾の <script> にインライン展開する。
- css/style.css はそのまま css.txt にコピー。
...
def main():
    ...
    (ROOT / "export" / "html.txt").write_text(html, encoding="utf-8")
    ...
    for f in ("html.txt", "css.txt"):
        p = ROOT / "export" / f
        print(f"{f:10} {p.stat().st_size / 1024:8.1f} KB")
